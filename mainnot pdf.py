...
#                 print(e)
            ...
            #     cmd=f'yt-dlp -o "{name}.mp4" "{url}"'    
                ...
                cmd=f'yt-dlp -o "{name}.mp4" "{url}"'
            elif ".pdf" or "pdf" in url: #and raw_text2 in ["144", "240", "360", "480", "720", "no"]:
                cmd = f'yt-dlp -o "{name}.pdf" "{url}"'
            elif "drive" in url:
                cmd = f'yt-dlp -o "{name}.pdf" "{url}"'
            elif raw_text2 == "no":# and raw_text2 in ["144", "240", "360", "480", "720", "no"]:
...
#                 if "drive" in url:
                    try:
                        ...
                        time.sleep(e.x)
                        continue
                elif cmd == f'yt-dlp -o "{name}.pdf" "{url}"' or ".pdf" or "pdf" in url:
...
@bot.on_message(filters.command(["top"])& ~filters.edited)
async def account_login(bot: Client, m: Message):
    editable = await m.reply_text("Hi i can currently download from **Vikramjeet & Sure60**\nSend txt file**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    await input.delete(True)
